...
import './App.scss';
import Field, { Mower, MowerMovement, MowerOrientation } from './components/Field';
import FileInput from './components/FileInput';

function App(): JSX.Element {
  const [field, setField] = useState({ width: 0, height: 0, mowers: [] as Mower[] });
  ...
  const loadConfigFile = (config: string) => {
    ...
    const mowerInstructions = instructions
      ...
      .map((instruction: string, index: number) => instruction + instructions[index * 2 + 2]);
    const mowers = mowerInstructions.map((mowerData: string) => {
      const [positionData, movementData] = mowerData.split(' ');
      const position = { x: parseInt(positionData[0]), y: parseInt(positionData[1]) }
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <FileInput onFileLoaded={(fileData: string) => loadConfigFile(fileData)} />
      </header>
    </div>
  );
}
